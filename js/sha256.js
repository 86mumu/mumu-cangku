// 纯 JS SHA-256：保证 http/https/file 等任意上下文哈希一致（与 Web Crypto 结果完全相同）
...
function sha256Hex(msg){
  ...
  const withOne=new Uint8Array(l+1); withOne.set(utf8); withOne[l]=0x80;
  const blocks=Math.ceil((l+9)/64);
  const total=blocks*64;
  const padded=new Uint8Array(total); padded.set(withOne);
...
if(typeof window!=="undefined")window.__sha256Hex=sha256Hex;
